...
async function main() {
  if (!utils.isAddress(ERC20_TOKEN_FACTORY_ADDRESS)) {
    throw new Error('Invalid ENV Variable ERC20_TOKEN_FACTORY_ADDRESS');
  }

  const TestTokenFactory = await ethers.getContractFactory('TestTokenFactory');
  const tokenFactory = TestTokenFactory.attach(ERC20_TOKEN_FACTORY_ADDRESS);
  console.log(
    'Connected to TestTokenFactory deployed at:',
  ...
  const tx = await tokenFactory.createTestToken(tokenName, tokenSymbol);
  const tokenAddress = await awaitTokenAddress(tx);
  const token = TestTokenUpgradeable.attach(tokenAddress);
  console.log(
    `Deployed a new ERC20 TestTokenUpgradable with name: "${tokenName}" and symbol: "${tokenSymbol}" at: ${token.address}`,
  );
